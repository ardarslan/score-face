...
def get_cfg() -> Dict[str, Any]:
    ...
    parser.add_argument("--order_views", type=str, required=True, help="When this is 'true', the views are ordered so that the optimization starts with the closest global rotation to the one in the original image, and it ends with the furthest one.")
    parser.add_argument("--two_rounds", type=str, required=True, help="When this is 'true', the pipeline runs two optimization rounds. In the first round, if a pixel is too dark (gray_intensity < min_gray_intensity_in_texture), then it is not copied to the small texture. In the second round, it is copied even though the pixel is dark. When this is 'false', the pipeline runs only the second round.")
    ...
    parser.add_argument("--device", type=str, default="cuda:1", choices=["cuda:1", "cpu"], help="Device to use.")
    ...
    parser.add_argument("--num_pixels_to_check_in_each_direction", type=int, default=5, help="This is used only if 'two_rounds' argument is 'true'. This argument corresponds to the number of pixels we check in each direction (up, down, left, right and diagonals), in order to understand whether a pixel on mesh was assigned a dark color due to being close to the pixels of background.")
    ...
    parser.add_argument("--step_elev", type=float, default=5.0, help="Elev step used during the optimization process.")
    ...
    parser.add_argument("--step_azimuth", type=float, default=5.0, help="Azimuth step used during the optimization process.")
...
def get_dark_pixel_allowances(cfg: Dict[str, Any]) -> List[bool]:
    two_rounds = cfg["two_rounds"]

    if two_rounds.lower() == "true":
        dark_pixel_allowances = [False, True]
    elif two_rounds.lower() == "false":
        dark_pixel_allowances = [True]
    return dark_pixel_allowances
