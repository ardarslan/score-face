from utils import get_experiment_name, save_cfg, set_seeds, get_initial_texture, update_texture, get_target_background
...
azimuths = [-60.0, -50.0, -40.0, -30.0, -20.0, 20.0, -10.0, 10.0, 0.0]
...
with torch.no_grad():
    for update_round, elev, azimuth in tqdm(loop_tuples):
        ...
        current_unoptimized_face = renderer.render_prerendering_results(current_texture, target_background, pixel_uvs, background_mask)
        ...
        current_optimized_face = renderer.render_prerendering_results(texture=current_texture, background=target_background, pixel_uvs=pixel_uvs, background_mask=background_mask)
    ...
    save_outputs(cfg=cfg, final_texture=current_texture, target_background=target_background, renderer=renderer, elevs=elevs, azimuths=azimuths)
