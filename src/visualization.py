...
from transformation import get_quaternion_final, get_quaternion_elev_azimuth
from typing import Dict, Any, List


def save_images(cfg: Dict[str, Any], _images: torch.Tensor, image_type: str, iteration: int, dark_pixel_allowance: bool, elev: float, azimuth: float) -> None:
    saved_image_size = cfg["saved_image_size"]
    ...
    image_main_folder_path = os.path.join(experiment_dir, "intermediates", image_type, f"darkpix_{dark_pixel_allowance}_elev_{elev}_azimuth_{azimuth}")
...
def save_optimization_gif(cfg: Dict[str, Any], dark_pixel_allowance: bool, elev: float, azimuth: float) -> None:
    experiment_dir = get_experiment_dir(cfg=cfg)
    face_main_folder_path = os.path.join(experiment_dir, "intermediates", "face", f"darkpix_{dark_pixel_allowance}_elev_{elev}_azimuth_{azimuth}")
...
def save_outputs(cfg: Dict[str, Any], final_texture: torch.Tensor, target_background: torch.Tensor, renderer: Renderer, elevs: List[float], azimuths: List[float], inverse_of_initial_quaternion: torch.Tensor) -> None:
    experiment_dir = get_experiment_dir(cfg=cfg)
    ...
    for index, (elev, azimuth) in enumerate(elevs_azimuths):
        ...
        pixel_uvs, background_mask = renderer.prerender(quaternion=quaternion_final)
